...
tf.flags.DEFINE_integer("epoch", 10, "number of training epoch")
...
tf.flags.DEFINE_string("output", "./output.json", "output file")
tf.flags.DEFINE_string("model_type", "CaptionGeneratorMyBasic", "the model type, inculding CaptionGeneratorBasic, CaptionGeneratorMyBasic, CaptionGeneratorSS, (default: CaptionGeneratorSS)")
tf.flags.DEFINE_string("meta_path", "./meta.dat", "training data parameter for test data")
...
class CapGenModel(object):
	...
	def dump_meta(self):
		...
		cPickle.dump(meta, open(FLAGS.meta_path, 'wb'))
	...
	def train(self):
		...
		with self.sess.as_default():
			...
			"""
			"""
			feats, test_id = data_utils.load_test_data(FLAGS.test_id, FLAGS.test_dir)
	...
	def inference(self, encoder_in, task=False):
		...
		if task:	
			...
			print(sentences[4])
			print("-------------------")
			with open('{}_Greedy.txt'.format(FLAGS.hidden), 'w') as f:
				f.write("BLEU score {}".format(score/len(sentences)))
	...
	def BeamSearch(self, encoder_in, task=False):
		...
		if task:
			...
			print(sentences[4])
			print("-------------------")
			with open('{}_Beam.txt'.format(FLAGS.hidden), 'w') as f:
				f.write("BLEU score {}".format(score/len(sentences)))

		return sentences

def load_model(sess, checkpoint_file, vocab_processor):
	meta = cPickle.load(open(FLAGS.meta_path, 'rb'))
	model = classmap[FLAGS.model_type](hidden_size=FLAGS.hidden, 
									vocab_size=len(vocab_processor._reverse_mapping), 
									encoder_in_size=meta["encoder_in_size"], 
									encoder_in_length=meta["encoder_in_length"],
									decoder_in_length=meta["decoder_in_length"], 
									word2vec_weight=None,
									embedding_size=FLAGS.embedding_dim,
									neg_sample_num=None,
									start_id=vocab_processor._mapping['<BOS>'],
									end_id=vocab_processor._mapping['<EOS>'],
									Bk=meta["Bk"])
...
def greedy_inference(sess, model, encoder_in, vocab_processor):
	feed_dict = {
		model.e_in:encoder_in,
	}
	preds = sess.run(model.de_outputs_infer, feed_dict=feed_dict)
	sentences = []
	...
	for ps in preds:
		...
		for w in ps:
			idx = np.argmax(w)
			vocab = vocab_processor._reverse_mapping[idx]
			if vocab == '<EOS>':
				break
			s.append(vocab)
		sentences.append(' '.join(s))
...
def beam_search(sess, model, encoder_in, vocab_processor):
	feed_dict = {
		model.e_in:encoder_in,
	}
	step_max_words, chosen_idx, chosen_idx_f = sess.run([model.step_max_words, model.chosen_idx, model.chosen_idx_f], feed_dict=feed_dict)
	...
	for result in beam_result:
		s_id = result.get_path()
		s_word = [vocab_processor._reverse_mapping[idx] for idx in s_id if idx != vocab_processor._mapping["<EOS>"]]
		sentences.append(' '.join(s_word))
