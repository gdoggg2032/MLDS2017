...
tf.flags.DEFINE_string("model_type", "dcwgan", "model type: [dcgan, dcwgan, dclsgan, mydcgan]")
...
def select_model(dm, model_type):

	if model_type == 'dcwgan':
	...
	elif model_type == 'dclsgan':
		from cond_wgan import ConditionalLSGAN

		model = ConditionalLSGAN(
			FLAGS.z_dim, 2 * dm.vocab.vocab_size, FLAGS.learning_rate, FLAGS.scale, FLAGS.generator_output_layer)

	elif model_type == 'mydcgan':
		from cond_wgan import MyConditionalGAN

		model = MyConditionalGAN(
			FLAGS.z_dim, 2 * dm.vocab.vocab_size, FLAGS.learning_rate, FLAGS.scale, FLAGS.generator_output_layer)
...
def train():
	...
	with tf.Graph().as_default():
		...
		model = select_model(dm, FLAGS.model_type)
		...
		with sv.managed_session(config=config) as sess:
			...
			for epoch in range(FLAGS.epochs):
				...
				os.system("cp -r {} {}_{}_{}".format(FLAGS.log, FLAGS.log, FLAGS.model_type, epoch))
